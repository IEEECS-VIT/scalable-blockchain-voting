...
describe("frontend demo files", function () {
  it("contains every required demo page with visible trust-boundary labels", async function () {
    ...
    assert.match(indexHtml, /Simulation boundary/i);
    assert.match(indexHtml, /not claiming/i);

    for (const page of manifest.pages) {
      const html = await readFile(join(demoDir, page.path), "utf8");
      assert.match(html, new RegExp(`${page.label} page`, "i"));
      assert.match(html, /Simulation boundary|Failure demonstration|Remaining blockers/i);
      assert.match(html, /placeholder|pending|not a real|not an Anon Aadhaar|not trustless|not an audited|mock/i);
      assert.match(html, /Demo home/);
    }
  });
});
